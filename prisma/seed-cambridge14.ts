...
function addArticle(
  ...
  difficulty: string, questions: { type: string; questionText: string; options: string[]; correctAnswer: string }[]
) {
  const id = cuid();
  db.prepare(
    "INSERT OR IGNORE INTO Article (id, title, titleCn, content, translation, difficulty, category, createdAt) VALUES (?, ?, ?, ?, ?, ?, ?, ?)"
  ).run(id, title, titleCn, content, translation, difficulty, "academic", now());
